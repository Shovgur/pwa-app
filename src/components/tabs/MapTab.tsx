  ...
  return (
    <div className="dashboard-page">
      ...
      <div className="dashboard-map-layout">
        {/* Карта + поиск */}
        <div className="dashboard-map-main" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}>
            <div style={{ position: 'relative' }}>
              ...
              <input placeholder="Поиск по адресу или метро..."
                style={{ width: '100%', padding: '13px 14px 13px 40px', borderRadius: 14, background: '#222D3F', border: '1px solid rgba(255,255,255,0.08)', color: '#f1f5f9', fontSize: 14, outline: 'none', boxSizing: 'border-box', fontFamily: 'inherit' }} />
            </div>
          </motion.div>

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}>
            <div style={{ borderRadius: 20, overflow: 'hidden', position: 'relative', height: 420, border: '1px solid rgba(255,255,255,0.08)' }}>
              <div style={{
        ...
        {/* Правая колонка: выбранный пин + районы */}
        <div className="dashboard-map-side" style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          {selected ? (
            <motion.div className="card" style={{ padding: '14px 16px' }}
              ...
              <div style={{ marginTop: 12, fontSize: 12, padding: '5px 12px', borderRadius: 100, fontWeight: 600, textAlign: 'center', background: selected.available ? 'rgba(34,197,94,0.15)' : 'rgba(239,68,68,0.12)', color: selected.available ? '#22c55e' : '#f87171' }}>
                {selected.available ? 'Свободно' : 'Занято'}
              </div>
            </motion.div>
          ) : (
            <div className="card" style={{ padding: '18px 16px', textAlign: 'center', color: '#64748b', fontSize: 13 }}>
              Выберите метку на карте, чтобы увидеть детали площадки
            </div>
          )}

          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }}>
            <h2 style={{ fontSize: 15, fontWeight: 700, color: '#f1f5f9', marginBottom: 12 }}>Площадки по районам</h2>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {AREAS.map((area, i) => (
                <motion.div key={area.name} className="card card-hover"
                  ...
                  whileHover={{ x: 2 }}>
                  <div style={{ width: 36, height: 36, borderRadius: 12, background: `${area.color}20`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    <MapPin size={16} color={area.color} />
                  </div>
                  <div style={{ flex: 1 }}>
